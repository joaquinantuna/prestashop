...
<div class="table_block" style="margin-top:1em">
...
<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" id="forgot_password">
	<fieldset>
		<p class="text">
			<label for="email">{l s='E-mail:'}</label>
			<input type="text" class="text" id="email" name="email" value="{if isset($smarty.post.email)}{$smarty.post.email|escape:'htmlall':'UTF-8'|stripslashes}{/if}" />
		</p>
		<p class="submit_pwd">
			<input type="submit" id="submitPassword" value="{l s='Retrieve Password'}" />
		</p>
	</fieldset>
</form>
{/if}
