		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				<td class="history_link bold">
					...
					<a class="color-myaccount" href="javascript:showOrder(1, {$order.id_order|intval}, 'order-detail');">
                    <br /><img src="{$img_dir}bullet.gif" alt="{l s='details'}" title="{l s='details'}" />
                    <b style="color:black">{l s='details'}</b>
                    </a>
				</td>
				...
				<td class="history_detail">
					<a href="{$base_dir_ssl}{if isset($opc) && $opc}order-opc{else}order{/if}.php?submitReorder&id_order={$order.id_order|intval}" title="{l s='Reorder'}">
                     {l s='Reorder'}
 					 <img src="{$img_dir}icon/cancel.png" alt="{l s='Reorder'}" title="{l s='Reorder'}" class="icon" />
					</a>
				</td>
			</tr>
		{/foreach}
