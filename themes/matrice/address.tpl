...
<p>{if isset($id_address)}{l s='Modify address'} {if isset($smarty.post.alias)}"{$smarty.post.alias}"{elseif isset($address->alias)}"{$address->alias|escape:'htmlall':'UTF-8'}"{/if}{else}{l s='To add a new address, please fill out the form below.'}{/if}</p>
 <p class="dash">&#8212;&#8212;</p>
...
<form action="{$link->getPageLink('address.php', true)}" method="post" class="std" id="add_update_adress">
	<fieldset>
		...
		<p style="background:#EEEEEE;padding:6px;margin:0;width:364px"><span for="alias">{l s='Assign an address title for future reference'}</span></p>
			<input type="text" id="alias" name="alias" value="{if isset($smarty.post.alias)}{$smarty.post.alias}{elseif isset($address->alias)}{$address->alias|escape:'htmlall':'UTF-8'}{elseif isset($select_address)}{else}{l s='My address'}{/if}" style="width:364px" />
			<sup>*</sup>
		</p>
		<p class="text">
	...
	{/if}
		...
		<p class="textarea">
			...
			<textarea id="other" name="other" cols="26" rows="3"  style="width:367px">{if isset($smarty.post.other)}{$smarty.post.other}{else}{if isset($address->other)}{$address->other|escape:'htmlall':'UTF-8'}{/if}{/if}</textarea>
		</p>
